...
class DWT_block(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        cA, (cH, cV, cD) = coeffs[0], (coeffs[1][0][:, :, 0, :, :], coeffs[1][0][:, :, 1, :, :], coeffs[1][0][:, :, 2, :, :])
        hf = self.high_conv(torch.cat((cH, cV, cD), dim=1))  
        lf = self.low_conv(cA)

        # 确保hf和lf的尺寸一致
        if hf.size()[2:] != lf.size()[2:]:
            # 将lf插值到与hf相同尺寸
            lf = F.interpolate(lf, size=hf.size()[2:], mode='bilinear', align_corners=False)
        
        # 使用 PyTorch 在相同设备上进行平滑（避免 cpu<->gpu 拷贝和 numpy/cv2）
        # 构造一个简单的高斯核（归一化）并对每个通道做分组卷积
        if hf.dtype != torch.float32:
            hf = hf.type(torch.float32)
        kernel = torch.tensor([[1., 2., 1.], [2., 4., 2.], [1., 2., 1.]], device=hf.device, dtype=hf.dtype)
        kernel = kernel.unsqueeze(0).unsqueeze(0)
        kernel = kernel / kernel.sum()
        kernel = kernel.repeat(hf.size(1), 1, 1, 1)  # (C,1,3,3) for grouped conv
        hf = F.conv2d(hf, kernel, padding=1, groups=hf.size(1))
        ...
        if hf.size()[2:] != lf.size()[2:]:
            # 将较小的特征图插值到较大特征图的尺寸
            if hf.size(2) * hf.size(3) > lf.size(2) * lf.size(3):
                lf = F.interpolate(lf, size=hf.size()[2:], mode='bilinear', align_corners=False)
            else:
        ...
        # 调整cat_hf_lf的尺寸以匹配tmp12
        if cat_hf_lf.size()[2:] != tmp12.size()[2:]:
            cat_hf_lf = F.interpolate(cat_hf_lf, size=tmp12.size()[2:], mode='bilinear', align_corners=False)
        ...
        # 确保输出尺寸与输入特征图f3尺寸一致
        if f13.size()[2:] != f3.size()[2:]:
            f13 = F.interpolate(f13, size=f3.size()[2:], mode='bilinear', align_corners=False)
